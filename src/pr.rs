...
pub fn run(args: &[OsString]) -> AppResult<()> {
    if !command_exists("gh") {
        return Err(
            "Error: 'gh' CLI not found. Install it from https://cli.github.com".to_string(),
        );
    }
    ...
    if action.as_deref() == Some("create") {
        ...
        if result.exit_code != 0 {
            ...
            if stderr.contains("no pull requests found") {
                return Err(format!(
                    "No PR found for branch '{}'. Use 'gwt pr create' to create one.",
                    branch
                ));
            }
    ...
    while index < args.len() {
        match arg_to_str(&args[index])? {
            ...
            value if value.starts_with('-') => {
                return Err(format!("Error: unknown pr option '{value}'"));
            }
            value => {
