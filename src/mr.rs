...
pub fn run(args: &[OsString]) -> AppResult<()> {
    if !command_exists("glab") {
        return Err(
            "Error: 'glab' CLI not found. Install it from https://gitlab.com/gitlab-org/cli"
                .to_string(),
        );
    }
    ...
    if action.as_deref() == Some("create") {
        ...
        let result = run_command(
            "glab",
            &["mr", "create", "--web", "--source-branch", &branch],
            None,
        )?;
        ...
        if result.exit_code != 0 {
            ...
            {
                return Err(format!(
                    "No MR found for branch '{}'. Use 'gwt mr create' to create one.",
                    branch
                ));
            }
    ...
    while index < args.len() {
        match arg_to_str(&args[index])? {
            ...
            value if value.starts_with('-') => {
                return Err(format!("Error: unknown mr option '{value}'"));
            }
            value => {
